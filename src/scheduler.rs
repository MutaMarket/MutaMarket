...
//! bids, the PLEX market history, character assets and contracts, the
//! stale-asset-import sweeper, and the public structure sweep. On by
//! default like the legacy scheduler; set `SCHEDULER_ENABLED=false` to opt
//! out (e.g. to avoid the ESI traffic during development —
...
pub fn start(pool: PgPool, reference: Arc<ReferenceData>, esi: EsiClient, sso: SsoClient) {
    {
        let pool = pool.clone();
...

    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for character_id in characters {
                    match assets::sync_character_assets(&pool, &reference, &esi, &sso, character_id)
                        .await
                    {
                        Ok(stats) => println!(
