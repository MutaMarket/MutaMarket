...
`sort/date-added/desc` orders by when a module's current public listing
appeared on MutaMarket, whether a contract or a direct listing. That order is
append-only, so polling the first page shows newly listed modules without
walking every page.
