...
async fn omega_calculator_serves_the_env_driven_sales() {
    let _markeedragon = EnvGuard::capture(mutamarket::server::omega::MARKEEDRAGON_SALE_ENV);
    let _evestore = EnvGuard::capture(mutamarket::server::omega::EVESTORE_SALE_ENV);
