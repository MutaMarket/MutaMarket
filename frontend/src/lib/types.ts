...
export interface CurrentUser {
   ...
   * only thing that turns ads off. */
  is_patreon_member: boolean;
  /** The account's accent color (`#rrggbb`) retinting the theme; null
...
export interface ModuleDetail {
  ...
  /** The signed-in user's private note. Absent for guests, null when
   * the user has no note on this module. */
  note?: NoteRef | null;
...
export interface CollectionNoteRef {
  collection: NoteCollectionRef;
  id: number;
  content: string;
}

/** The collection a note hangs off, the legacy CollectionResource with
 * only its own columns (no counted or loaded relations). */
