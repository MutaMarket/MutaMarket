...
describe("editRequest", () => {
  it("is null when nothing changed", () => {
    ...
    expect(
      editRequest(session({ drafts: { 1: { value: "a", stored: "a" } } })),
    ).toBeNull();
  });
...
describe("isValid", () => {
  it("only guards price drafts", () => {
    expect(
      isValid(session({ drafts: { 1: { value: "anything", stored: "" } } })),
    ).toBe(true);
    expect(
      isValid(
        session({ mode: "price", drafts: { 1: { value: "abc", stored: "" } } }),
      ),
    ).toBe(false);
    expect(
      isValid(
        session({ mode: "price", drafts: { 1: { value: "10", stored: "" } } }),
      ),
    ).toBe(true);
  });
});
...
describe("permissions", () => {
  ...
  it("reads character ids off an untyped nav payload", () => {
    expect(navCharacterIds({ characters: [{ id: 1 }, { id: 2 }] })).toEqual([
      1, 2,
    ]);
...
describe("saveEdits", () => {
  it("posts the request and ends the session", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue({ ok: true, type: "basic", status: 200 });
  ...
  it("treats the endpoints referer redirect as success", async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue({ ok: false, type: "opaqueredirect", status: 0 }),
    );
    startEdit("note");
  ...
  it("keeps the session open when the post fails", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({ ok: false, type: "basic", status: 500 }),
    );
    startEdit("note");
    setDraft(module(), "written");
