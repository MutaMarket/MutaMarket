...
export function storedValue(mode: EditMode, module: ModuleDetail): string {
  switch (mode) {
    ...
    case "price": {
      const price = module.public_asset?.price ?? null;
...
export function navCharacterIds(nav: unknown): number[] {
  return ((nav as NavState | null | undefined)?.characters ?? []).map(
...
export function isValid(session: EditSession): boolean {
  return (
    session.mode !== "price" ||
    Object.values(session.drafts).every(
      (draft) => parsePrice(draft.value) !== null,
    )
  );
}
