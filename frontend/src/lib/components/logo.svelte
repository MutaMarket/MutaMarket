<script lang="ts">
  // The MutaMarket mark, the legacy Icons/Logo.vue verbatim: an inline
  // SVG filled with currentColor, so it takes whatever text color the
  // parent sets (text-primary in the nav follows the accent
  // automatically).
  let { class: className = '' }: { class?: string } = $props();
</script>

<svg
  width="100%"
  height="100%"
  viewBox="0 0 394 217"
  version="1.1"
  xmlns="http://www.w3.org/2000/svg"
  xml:space="preserve"
  class={className}
>
  <g transform="matrix(0.1,0,0,-0.1,-103,408.3)">
    <path
      d="M2386,4080C2385,4080 1030,1928 1030,1925C1030,1922 2658,1920 2658,1920C2658,1920 2920,2325 2920,2329C2920,2333 2676,2690 2674,2690C2673,2690 2416,2320 2416,2320C2416,2320 1770,2323 1770,2326C1770,2334 2395,3330 2398,3330C2402,3330 3345,1920 3345,1920C3345,1920 4970,1923 4970,1926C4970,1929 3611,4083 3611,4083C3611,4083 3086,3281 3084,3275C3081,3269 3318,2920 3322,2920C3329,2920 3595,3330 3600,3330C3604,3330 4261,2290 4261,2290L3600,2290C3600,2290 2387,4080 2386,4080Z"
      style="fill-rule: nonzero"
      class="fill-current"
    />
  </g>
</svg>
