...
describe('donations-list', () => {
  ...
  it('renders a repeat donor, whose row carries a tooltip', async () => {
    const { container } = await render(DonationsList, {
  ...
  it('renders a one-off donor, whose row has no tooltip', async () => {
    const { container } = await render(DonationsList, {
  ...
  it('renders an empty list without complaint', async () => {
    const { container } = await render(DonationsList, { donations: [], showRank: true });
