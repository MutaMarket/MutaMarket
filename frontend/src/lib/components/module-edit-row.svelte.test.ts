...
describe("the note row", () => {
  ...
  it("shows a stored note as text", async () => {
    ...
    await expect
      .element(screen.getByText("watch this roll"))
      .toBeInTheDocument();
  });
...
describe("the asking-price row", () => {
  it("only appears while prices are being edited", async () => {
    ...
    startEdit("price");
    await expect
      .element(screen.getByLabelText("Asking price"))
      .toHaveValue("1500000000");
  });

  it("formats what was typed and flags a value that is not a number", async () => {
    const listed = module({
    ...
    await expect
      .element(screen.getByText("1.5 billion ISK"))
      .toBeInTheDocument();

    setDraft(listed, "");
    await expect.element(screen.getByText("No price")).toBeInTheDocument();

    setDraft(listed, "soon");
    await expect.element(screen.getByText("Not a number")).toBeInTheDocument();
  });
});
