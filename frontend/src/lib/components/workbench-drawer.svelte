<script lang="ts">
	...
	function sortCompareBy(key: number | 'module') {
		compareSort = compareSort?.key === key ? { key, desc: !compareSort.desc } : { key, desc: true };
	}
		...
		if (compareSort !== null) {
			...
			const key = (entry: WorkbenchEntry) =>
				sort.key === 'module'
					...
					: (entry.module.mutated_attributes.find((attribute) => attribute.id === sort.key)
				...
				{:else if view === 'compare' && compare !== null}
					<div class="px-1">
						{#if compareTypes.length > 1}
							<div
								class="mb-2 flex rounded-[7px] border border-border bg-card-2 p-0.5 self-start w-max"
							>
				...
				{:else}
					...
					<div class="grid grid-cols-[repeat(auto-fill,minmax(270px,1fr))] gap-4 pb-2">
						{#each entries as entry (entry.id)}
							<div
								class="relative"
								style="grid-row: span {2 + entry.module.mutated_attributes.filter(isVisual).length}"
							>
								<button
									type="button"
