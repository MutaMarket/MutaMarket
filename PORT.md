...
|---|---|---|---|---|
| GET `/` | ModuleController@index | Modules browser page; for-sale modules (`hasLatestContract` OR public assets), `withDefaultRelations`, `simplePaginate(40)`; props: `modules`, `search`, `available_types`, `stats` | **PARTIAL** — grid + filters + market stats strip render; no pagination links/meta, no `available_types`, no list/table modes | `frontend/src/lib/components/module-browser.svelte`, `src/server/api.rs` |
| GET `/modules/add` | ModuleController@create | "Add module" page (paste PYFA / showinfo link) | **STUB** — placeholder page | `frontend/src/lib/components/placeholder.svelte` |
| GET `/modules/{module}` | ModuleController@show | Single module detail page; module with all relations, `source_type_comparisons`, probability data | **PARTIAL** — card + basic detail; no source-type comparison table, no probability/estimator sidebar | `frontend/src/lib/components/module-browser.svelte`, `src/server/api.rs` |
| GET `/modules/{query?}` | ModuleController@index | Same as `/` with filter segments | **PARTIAL** — as `/` | `frontend/src/lib/components/module-browser.svelte`, `src/server/api.rs` |
| GET `/all-modules/{query?}` | AllModulesController@index | Browser incl. modules with no live contract | **PARTIAL** — renders; pagination/stats gaps as `/` | `frontend/src/routes/all-modules`, `src/server/api.rs` |
| GET `/characters` | CharacterController@index | `Characters/ShowAllCharactersPage`; characters with public ownerships, premium-first, `paginate(32)`; props `characters`, `search` | **PARTIAL** — index renders; no pagination, no search box UI | `frontend/src/routes/characters`+`collections`, `src/server/social.rs` |
| GET `/characters/{character}/{query?}` | CharacterController@show | `Characters/ShowCharacterPage`; props `character`, `modules`, `search`, `stats`, `available_types`, `locations` (owner only) | **PARTIAL** — character + owned modules; no stats, available_types, locations, `created` mode toggle | `frontend/src/routes/characters`+`collections`, `src/server/social.rs` |
| GET `/collections` | CollectionController@index | `Collections/ShowAllCollectionsPage`; `public_collections` (paginate 12) + `personal_collections` | **PARTIAL** — public index; no personal list, no pagination | `frontend/src/routes/characters`+`collections`, `src/server/social.rs` |
| GET `/collections/{collection}/{query?}` | CollectionController@show | `Collections/ShowCollectionPage`; `collection`, `modules`, `search`, `available_types`, `stats`, `locations`; policy: private = owner only | **PARTIAL** — modules render, 403 policy works; no stats/available_types/locations/auto-sync UI | `frontend/src/routes/characters`+`collections`, `src/server/social.rs` |
| GET `/calculator/{query?}` | CalculatorController@index | Roll calculator page (pick type + mutaplasmid, live roll math) | **STUB** — placeholder; mutation math is ported and reusable | `frontend/src/lib/components/placeholder.svelte` |
| GET `/statistics/{query?}` | StatisticsController@index | Statistics page (market-wide roll/price stats) | **STUB** — placeholder | `frontend/src/lib/components/placeholder.svelte` |
| GET `/premium` | PremiumController@index | Premium info / purchase page | **STUB** — placeholder | `frontend/src/lib/components/placeholder.svelte` |
...
| GET `/{query?}` | NotFoundController@index | Catch-all 404 page | **DONE** | JSON 404 (`src/server/mod.rs`) + `frontend/src/routes/+error.svelte` |

### 1.2 Authenticated pages

| Method · Path | Controller@action | Response | Status | Impl |
|---|---|---|---|---|
| GET `/personal/modules/{query?}` | PersonalModuleController@index | Owned modules (assets ∪ contract items), `simplePaginate(40)`, `asset_import` prop | **PARTIAL** — grid + live import panel over `/ws`; no filter sidebar, no pagination | `frontend/src/routes/personal/modules`, `src/server/personal.rs` |
